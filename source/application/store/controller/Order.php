...
class Order extends Controller{
     ...
     * Time: 20:04
     */
    public function state($order_sn = 0,$store_id = 0){
        $orderModel = new OrderModel();
        if($orderModel->acceptOrder($order_sn,$store_id,$this->yoshop_store['user']['store_user_id'])){
            return $this->renderSuccess('接单成功');
        }
        return $this->renderError($orderModel->getError() ? : '接单失败');
