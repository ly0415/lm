...
class GoodsModel extends Controller
{
    ...
    public function add(){
        ...
        if(!$this->request->isAjax()){
            //获取分类
            $category = GoodsCategory::getCate();
...
//        dump($model->toArray());die;
        if (!$this->request->isAjax()) {
            ...
            isset($model['cate_path_id'][2]) && $category_2 = GoodsCategoryModel::getCate($model['cate_path_id'][2]);
            isset($model['cate_path_id'][1]) && $category_3 = GoodsCategoryModel::getCate($model['cate_path_id'][1]);
            return $this->fetch('edit', compact('model','list','category_1','category_2','category_3'));
        }
        // 更新记录
        if ($model->edit($this->postData('goods_model'))) {
            return $this->renderSuccess('更新成功', url('goods.goods_model/index'));
        }
