...
class StoreGoods extends StoreGoodsModel{
    ...
    public function getPackageSpecData($goods_id){
        $spec_key   = Db::name('store_goods_spec_price')->field('key')->where(['store_goods_id'=>$goods_id])->select();
        if(empty($spec_key)){
            return false;
        }else {
            static $result = [];
            ...
            foreach ($goods as $v){
                ...
                if(isset($v['is_joint']) && $v['is_joint'] == 1 && $v['store_goods_joint']){
                    foreach ($v['store_goods_joint'] as &$joint){
                        ...
                        if(!$this->checkAddStoreGoods($v['goods_name'],$joint['store_goods']['goods_id'],STORE_ID,$joint['store_goods']['goods_name'],$spec_arr,$joint['key_name'],$joint['num'],$joint)){
                            return false;
                        }
                    }
                }
    ...
    public function checkAddStoreGoods($joint_goods_name,$goods_id,$store_id,$goods_name,$key,$key_name,$num,&$joint){
        if(!$data = self::get(['goods_id'=>$goods_id,'store_id'=>$store_id,'mark'=>1])){
