<?php
    ...
    public function goodExist($store_goods_id,$item_id,$shipping_price,$user_id,$store_id,$goods_price,$fx_code,$shipping_store_id,$deliveryType){
        $query =array(
            'cond' =>"`goods_id` = '{$store_goods_id}' and `store_id` = '{$store_id}' and `spec_key` = '{$item_id}' and `user_id` = '{$user_id}'
             and `shipping_price` ='{$shipping_price}' and `goods_price` = '{$goods_price}' and `fx_code` = '{$fx_code}' and `shipping_store_id` = '{$shipping_store_id}' and `delivery_type` = '{$deliveryType}' AND `is_buy` = 0",
            'fields' =>'`id`'
        );
     ...
     public function getCartGoodsNum($store_goods_id,$item_id,$shipping_price,$user_id,$store_id,$goods_price,$shipping_store_id,$deliveryType){
         $query =array(
             'cond' =>"`goods_id` = '{$store_goods_id}' and `store_id` = '{$store_id}' and `spec_key` = '{$item_id}' and `user_id` = '{$user_id}' and `shipping_price` ='{$shipping_price}' and `goods_price` = '{$goods_price}' and `shipping_store_id` = '{$shipping_store_id}' and `delivery_type` = '{$deliveryType}' AND `is_buy` = 0 ",
             'fields' =>'`goods_num`,`id`'
         );
         $rs = $this->getOne($query);
         return $rs;
     ...
     * @date: 2017/9/19
     */
    public function addCart($goodInfo,$good_num,$type,$deliveryType){
        $orderMod = &m('order');
       if(!$this->goodExist($goodInfo['store_goods_id'],$goodInfo['item_id'],$goodInfo['shipping_price'],$goodInfo['user_id'],$goodInfo['store_id'],$goodInfo['goods_price'],$goodInfo['fx_code'],$goodInfo['shipping_store_id'],$goodInfo['delivery_type'])){
          if($deliveryType == 3 || $deliveryType == 4){
              $goods = $this->getGoodInfo($goodInfo['store_goods_id'],58);
          }else{
     ...
     public function getGoodById($store_id,$user_id){
       ...
       */
        public function getGoodNameById($store_goods_id,$lang_id){
            $sql = "SELECT goods_name from ".DB_PREFIX."store_goods ".
