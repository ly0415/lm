...
class RegionModuleMod extends BaseMod {
    ...
    public function getAuthList() {
        ...
        $sql = 'select group_concat(distinct parent_id) as pid from ' . DB_PREFIX . 'store_user_admin where level = 2 and id in (' . implode(',', $authChildIds) . ') group by level ';
        $authPids = $this->querySql($sql); //当前登录人拥有的一级菜单id
        $sql = 'select * from ' . DB_PREFIX . 'store_user_admin where id in (' . $authPids[0]['pid'] . ') and is_menu = 1 order by sort asc';
        $info = $this->querySql($sql); //当前登录人拥有的一级菜单详细信息
        foreach ($info as $k => $v) {
            $sql = 'select * from ' . DB_PREFIX . 'store_user_admin where parent_id = ' . $v['id'] . ' and id in (' . implode(',', $authChildIds) . ') and is_menu = 1 order by sort asc ';
