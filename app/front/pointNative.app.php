...
class pointNativeApp extends BaseFrontApp {
...
//        $wxConfig::$APPSECRET = "cb06dfe09354ada01688cea7173b3c45";
    ...
    public function native() {
        ...
        $rs = $this->pointOrderMod->isExist($order_id);
        if (empty($order_id)) {
        ...
        $input->SetNotify_url("http://www.711home.net/index/pointwxNotify/notify_{$this->storeid}.html");
        ...
        $this->display('native/pointOrder.html');
    }
...
//    }
    ...
    public function checkStatus() {
        ...
        $auxiliary = !empty($_REQUEST['auxiliary']) ? htmlspecialchars(trim($_REQUEST['auxiliary'])) : '0';
        $rs = $this->pointOrderMod->isExist($order_sn);
        if ($rs['status'] == 1) {
            $data = array(
                'status' => 1,
                'url' => "?app=userCenter&act=personPoint&storeid={$storeid}&lang={$lang}&auxiliary={$auxiliary}",
