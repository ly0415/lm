...
print_r($province);
        echo 3;
        print_r(implode('-',json_decode($province)));
        print_r($postal_code);
        print_r($city);die;

//        $province = !empty($_REQUEST['province']) ? json_encode(implode('-',explode(',',$_REQUEST['province']))): '';
...
//        var_dump($address);die;
        ...
        $info=array('langData'=>$langData,'addressData'=>$addressData);
        $this->setData($info,1,'');
    }
    /**
     * 提现申请页面
