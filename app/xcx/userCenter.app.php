 ...
 * @date:   2018-08-23
 */
class UserCenterApp extends BasePhApp
{
...
//赚取积分
            ...
            if (!empty($userData[0]['xcx_openid'])) {
                if ($userData[0]['xcx_openid'] != $openid) {
                    ...
                    if ($phoneCode == $smsCode) {
                        $s = "SELECT id FROM bs_user WHERE mark = 1 AND  phone=".$phone;
                ...
                if ($phoneCode == $smsCode) {
                    $sqls = "SELECT id FROM bs_user WHERE mark = 1 AND phone =".$phone;
...
//            }
            ...
            if (!empty($userData[0]['xcx_openid'])){
                if ($userData[0]['xcx_openid'] != $openid){
                    ...
                    if ($phoneCode == $smsCode) {
                        $s = "SELECT id,is_use FROM bs_user WHERE mark = 1 AND phone=".$phone;
                        $e = $this->userMod->querySql($s);
                        if ($e) {
                            if($e[0]['is_use'] == 2 || $e[0]['is_use'] == 3){
                                $this->setData(array(),0,'无登录权限！');
                            }
                            $arr = array(
                                'user_id' => $e[0]['id']
                            );
                ...
                $this->userMod->doEdit($userData[0]['id'],array('xcx_openid' => $openid));
                if ($phoneCode == $smsCode) {
                    $sqls = "SELECT id,is_use FROM bs_user WHERE mark = 1 AND phone =".$phone;
                    $ss = $this->userMod->querySql($sqls);
                    if ($ss) {
                        if($ss[0]['is_use'] == 2 || $ss[0]['is_use'] == 3){
                            $this->setData(array(),0,'无登录权限！');
                        }
                        $arr = array(
                            'user_id' => $ss[0]['id']
    ...
    */
    ...
    public function doAddress()
    {
        ...
        if (empty($mailing_address)) {
            $this->setData(array(), $status = '0', '请填写邮寄收货地址！'); //请填写收货地址
        }
