...
//            'goods_content' => $goods_content,
            ...
            'bar_code'=>$bar_code,
                    ...
                    // //删除关联商品
        $lme = array(
            'goods_name' => $goods_lang[29]['goods_name'],
            'goods_sn' => $goods_sn,
            'goods_original_image' => $original_img,
            'cate_id' => $cat_id,
            'spec_type' => $spec_type ? $spec_type : 20,
            'market_price' => $market_price,
            'content' => $goods_lang[29]['goods_content'],
            'goods_model_id' => $goods_type,
            'cost_price'=>$cost_price,
            'sale_price'=>$shop_price,
            // 'create_user'=>$this->accountId,
            // 'update_user'=>$this->accountId,
            // 'create_time' => time(),
            // 'update_time' => time()
...
//        echo '<pre>';print_r($data);die;
        ...
        if ($_POST['goods_id']) {
            ...
            $lme['goods_id'] = $good_id;
            $lme['update_user'] = $this->accountId;
            $lme['update_time'] = time();
            $sql_3 = $this->buildSql($lme,'update',$good_id);
            $goodMod->sql_b_spec($sql_3);
            if ($goods_type == 0) {
                //删除原规格
            ...
            $lme['goods_id'] = $good_id;
            $lme['create_user'] = $this->accountId;
            $lme['create_time'] = time();
            $sql_1 = $this->buildSql($lme);
            // echo $sql_1;die;
            $goodMod->sql_b_spec($sql_1);
            //生成二维码
            $code = $this->goodsZcode($good_id);
            $cond['code_url'] = $code;
            $goodMod->doEdit($good_id, $cond);
        ...
        if ($good_id || $res) {
            ...
            $sql_2 = $this->buildSql(array('goods_sn'=>$goods_sn),'update',$good_id);
            $goodMod->sql_b_spec($sql_2);
            //修改区域端商品
...
/*        public function updateUrl(){
    ...
    public function delGoodsLme($goods_id) {
        $sql = "delete from lmeri_goods where goods_id=" . $goods_id;
        $this->goodMod->sql_b_spec($sql);
    }
...
//        $attributeList = $GoodsAttribute->where("type_id = $type_id")->select();
   ...
   */
   public function buildSql($array, $type='insert', $goods_id, $table='lmeri_goods', $exclude = array()){
    ...
    if(count($array) > 0){
      ...
      if('insert' == $type){
        ...
        $sql = "update {$table} set " . implode(",", $temparr) . " where goods_id = " . $goods_id;
      }
    }
    return $sql;
  }
