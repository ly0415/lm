                ...
                methods: {
                    ...
                    onSubmitAddGroup: function () {
                        ...
                        if (_this.addGroupFrom.specName === '' || _this.addGroupFrom.specValue === '') {
                            layer.msg('请填写标签名或标签值');
                            return false;
                        }
                        // 添加到数据库
                        var load = layer.load();
                        $.post(STORE_URL + '/store.tag/add', {
                            tag_name: _this.addGroupFrom.specName,
                            tag_value: _this.addGroupFrom.specValue
                        }, function (result) {
                            layer.close(load);
                            if (result.code !== 1) {
                            ...
                            _this.spec_attr.push({
                                group_id: result.data['tag_id'],
                                group_name: _this.addGroupFrom.specName,
                                spec_items: [{
                                    item_id: result.data['tag_value_id'],
                    ...
                    onSubmitAddValue: function (index) {
                        ...
                        // 添加到数据库
                        var load = layer.load();
                        $.post(STORE_URL + '/store.tag/addTagValue', {
                            tag_id: specAttr.group_id,
                            tag_value: specAttr.tempValue
                        }, function (result) {
                            layer.close(load);
                            if (result.code !== 1) {
                                layer.msg(result.msg);
                    ...
                    onDeleteGroup: function (index) {
                        var _this = this;
                        layer.confirm('确定要删除该标签吗？确认后不可恢复请谨慎操作'
                            , function (layerIndex) {

                                var load = layer.load();
                                $.post(STORE_URL + '/store.tag/delTag', {
                                    tag_id: _this.spec_attr[index].group_id,
                                }, function (result) {
                                    layer.close(load);
                                    if (result.code !== 1) {
                                        layer.msg(result.msg);
                    ...
                    onDeleteValue: function (index, itemIndex) {
                        var _this = this;
                        layer.confirm('确定要删除该标签值吗？确认后不可恢复请谨慎操作'
                            , function (layerIndex) {
                                 var load = layer.load();
                                 console.log(_this.spec_attr[index]);
                                $.post(STORE_URL + '/store.tag/delTagValue', {
                                    tag_id: _this.spec_attr[index].group_id,
                                    tag_value_id: _this.spec_attr[index].spec_items[itemIndex].item_id
                                }, function (result) {
                                    layer.close(load);
                                    if (result.code !== 1) {
                                        layer.msg(result.msg);
