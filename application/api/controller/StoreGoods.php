...
class StoreGoods extends Controller{
     ...
     * @author  : luffy
     * @date    : 2019-11-12
     * http://localhost/lmeriPro/web/index.php?s=/api/store_goods/getSpec/store_goods_id/15012
     */
    public function getSpec($store_goods_id){
        if(empty($store_goods_id)){
            return $this->renderError('参数错误！');
        }
        $model      = new StoreGoodsModel;
        return $model->getStoreGoodsSpecData($store_goods_id);      //用老的方法
    }
     ...
     * @date    : 2019-11-12
     */
    public function getSpecPrice($store_goods_id, $spec_str){
        if(empty($store_goods_id) || empty($spec_str)){
            return $this->renderError('参数错误！');
        }
        $model      = new StoreGoodsModel;
        return $this->renderSuccess($model->getStoreGoodsSpecPrice($store_goods_id, $spec_str));
    }
