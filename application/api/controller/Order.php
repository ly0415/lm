...
//        $lang $data['cart_ids']= !empty($_REQUEST['lang']) ? intval($_REQUEST['lang']) : $this->langid; //语言
        ...
        $data['sendout']= !empty($_REQUEST['sendout']) ? $_REQUEST['sendout'] : ''; //配送方式 数组形式 商品id-配送方式
        $data['post_sendout']= !empty($_REQUEST['post_sendout']) ? $_REQUEST['post_sendout'] : ''; //
        $data['shippingfee']= !empty($_REQUEST['shippingfee']) ? htmlspecialchars($_REQUEST['shippingfee']) : 0; //邮费
        $data['fx_user_id']= !empty($_REQUEST['fx_user_id']) ? intval($_REQUEST['fx_user_id']) : ''; //分销用户id
//        $rule_id $data['cart_ids']= !empty($_REQUEST['rule_id']) ? intval($_REQUEST['rule_id']) : ''; //分销规则id
        $data['daifu']=!empty($_REQUEST['daifu']) ?$_REQUEST['daifu']:''; //是否代付
