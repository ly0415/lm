...
class StoreGoods extends StoreGoodsModel
{
    ...
    public function getStoreGoods($store_id, $rtid, $type, $page, $search_name, $type_id){
        ...
        if(isset($business_tree[$rtid])){
            ...
            foreach ($business as $k => $v){
                $result = $this->alias('a')
                    ->join('goods_auxiliary_class b', 'a.goods_sn = b.goods_sn')
        ...
        empty($search_name) && $this->where(['b.business_id' => $type_id]);
        ...
        $new_data = [];
        $list = $this->alias('a')
            ->field('a.id,a.goods_id,a.cat_id,a.store_id,a.shop_price,a.market_price,a.goods_name,a.original_img,a.goods_storage,a.attributes,a.room_id as room_type_id,a.delivery_fee,store_goods_type')
            ->where($where)
            ->limit(($page-1) * 20,20)
        ...
        //数据处理
        foreach ($list as $k => $v){
            if($v['store_goods_type'] ==  3){
                $v['original_img'] = 'web/uploads/small/'.$v['original_img'];
            }
            //商品三个价格
    ...
    public function  getStoreGoodsSpecData($store_goods_id){
        $resule = $first_key = $spec_price_info = [];
        //获取网格规格信息
        ...
        //同步扣除商品获取总站商品的store_goods_id即可
        $store_goods_info = $aaaModel::get($store_goods_id);
        if($store_goods_info['deduction'] == 1){
  	   $_info                  = $this->where(['goods_id'=>$store_goods_info['goods_id'], 'mark'=>1, 'store_id'=>58])->find();
            $store_goods_id         = $_info['id'];

            $store_goods_info       = $_info;
        }
        if(!empty($spec_arr)){
            ...
            return ['spec' => array_values($resule), 'price'=>$spec_price_info['price'], 'stock'=>$spec_price_info['goods_storage'], 'first_key'=>$first_key, 'store_goods_id'=>$store_goods_id];
        } else{
            return ['price'=>$store_goods_info['shop_price'], 'stock'=>$store_goods_info['goods_storage'], 'store_goods_id'=>$store_goods_id];
        }
    }
