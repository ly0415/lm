                            ...
                            <div class="widget am-cf">
                                <form id="my-form" class="am-form tpl-form-line-form" method="post">
                                    ...
                                    <div class="am-form-group">
                                        ...
                                        <div class="am-u-sm-8 am-u-end">
                                            <input id="marketValue" autocomplete="off" type="text" class="tpl-form-input" name="shop_price" value="<?= $details['shop_price'] ?>" placeholder="本店售价" required>
                                        </div>
                                    </div>
                                    <div class="am-form-group">
                                        <label for="marketValue" class="am-u-sm-2 am-form-label form-require"> 本店库存： </label>
                                        <div class="am-u-sm-8 am-u-end">
                                            <input id="marketValue" autocomplete="off" type="text" class="tpl-form-input" name="goods_storage" value="<?= $details['goods_storage'] ?>" disabled>
...
<!--                            <div class="widget-head am-cf">-->
<!--                                <button type="button" class="am-fr am-btn am-btn-secondary am-btn-xs" data-am-modal="{target: '#doc-modal-1'}">设置</button>-->
<!--                            </div>-->
                            <!-- <button type="button" class="am-btn am-btn-secondary am-btn-xs" data-am-modal="{target: '#doc-modal-1'}">设置</button> -->
                            <div class="widget am-cf">
...
<script>
    $(function () {
        ...
        $.post("<?= url('StoreGoods/getSpecInput') ?>", {goods_id: <?= $details['id'] ?>}, function (result) {
        ...
        $('body').on('change','.j-edit-data1',function(){
            $.post("<?= url('StoreGoods/edit') ?>", {goods_id: <?= $details['id'] ?>, value_data: $(this).val(), spec_key: $(this).attr('j-item-key'), type: 2, tp: 1}, function (result) {

            });
        });
        $('body').on('change','.j-edit-data2',function(){
            $.post("<?= url('StoreGoods/edit') ?>", {goods_id: <?= $details['id'] ?>, value_data: $(this).val(), spec_key: $(this).attr('j-item-key'), type: 2, tp: 2}, function (result) {

            });
        });
        $('body').on('change','.j-edit-data3',function(){
            $.post("<?= url('StoreGoods/edit') ?>", {goods_id: <?= $details['id'] ?>, value_data: $(this).val(), spec_key: $(this).attr('j-item-key'), type: 2, tp: 3}, function (result) {
