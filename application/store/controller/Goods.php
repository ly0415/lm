...
use app\store\model\GoodsBrand;
use app\store\model\GoodsStyle;
use Think\Db;
...
class Goods extends Controller
{
    ...
    public function batch(){
        $model = new StoreGoodsModel();
        if ($this->request->isAjax()) {
            if ($model->batch($this->postData('batch'))) {
                return $this->renderSuccess('批量修改成功');
            }
            return $this->renderError($model->getError() ?: '批量修改失败');
        }
    }
     ...
     * Time: 15:43
     */
    public function _batch($goods_id){
        $model = new StoreGoodsModel();
        //获取商品存在的规格
        $list   = $model -> getStoreGoodsSpecData($goods_id);
        $this->view->engine->layout(false);
        return $this->fetch('_batch', compact('list'));
    }
...
//        dump($store_goods);die;
    ...
    public function cafe(){
        if (!$this->request->isAjax()) {
            $category = GoodsCategoryModel::getGoodsCategoryByPid(0);
            $brand = (new GoodsBrand())->getList();
            $style = (new GoodsStyle())->getList();
            return $this->fetch('cafe',compact('category','brand','style'));
        }
        $model = new StoreGoodsModel();
        if ($model->cafe($this->postData('cafe'))) {
            return $this->renderSuccess('添加成功', url('goods/index'));
        }
        return $this->renderError($model->getError() ?: '添加失败');
    }
     ...
     * Time: 15:40
     */
    public function set_model($id){
        $model = StoreGoodsModel::detail($id);
        if (!$this->request->isAjax()) {
            // 商品模型
            $category = (new GoodsModelModel)->getListAll();
            return $this->fetch('set_model',compact('category'));
        }
        dump($model->toArray());die;
        if ($model->set_model($this->postData('model'))) {
            return $this->renderSuccess('添加成功', url('goods/index'));
        }
        return $this->renderError($model->getError() ?: '添加失败');
    }
