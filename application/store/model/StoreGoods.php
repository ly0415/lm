...
class StoreGoods extends StoreGoodsModel{
    ...
    public function getList($is_list = false, $business_id = 0, $category_id = 0, $search = '', $status = null, $goods_sn = '',$attributes = 0){
        ...
        if($is_list && T_GENERAL){
            $this->join('bs_business b', 'a.room_id = b.id','LEFT');
            $this->where('b.pid', '=', BUSINESS_ID);
        }
        //根据二级业务类型查找数据，囊括辅助分类
        if($business_id > 0){
            $this->field('b.business_id');
            $this->join('goods_auxiliary_class b', 'a.goods_sn = b.goods_sn');
        ...
        // 执行查询
        $list = $this->field('a.id,a.goods_id,a.cat_id,a.goods_sn,a.market_price,a.shop_price,a.goods_name,a.goods_storage,a.is_on_sale,a.original_img,a.sort,a.attributes,a.add_time,a.room_id,a.deduction,store_goods_type')->alias('a')
            ->where('a.mark', '=', 1)
            ->where($filter)
...
//                dump($this->getLastSql());die;
    ...
    public function toSwitch($value){
        ...
        if($value['store_goods_type'] == 3){
            $value['original_img']        = 'web/uploads/small/'.$value['original_img'];
        }
...
//        if($goods_info){
            //业务类型
            $value['format_business_name']  = Db::name('business')->where(['id'=>$value['room_id']])->value('name');
            //库存扣除方式
            $value['format_deduction']      = ($value['deduction'] ? $GoodsModel->deduction[$value['deduction']] : '');
            //配送属性--获取门店商品的
...
//        }
        ...
        if(isset($value['room_id'])){
            ...
            $auxiliarys                     = Db::name('goods_auxiliary_class')->where(['goods_sn'=>$value['goods_sn'], 'business_id'=>['neq', $value['room_id']]])->select();
        }
        if(!empty($auxiliarys)){
            $bus        = [];
    ...
    public function getPackageSpecData($goods_id){
        $spec_key   = Db::name('store_goods_spec_price')->field('key')->where(['store_goods_id'=>$goods_id, 'mark'=>1])->select()->toArray();
        if(empty($spec_key)){
            return false;
        }else {
            static $result = [];
    ...
    public function add($data){
        ...
        try{
            foreach ($goods as $v){
                ...
                //总站商品和对应规格数据新增
                if(isset($v['store_goods_type']) && $v['store_goods_type'] == 3 && $v['store_goods_joint']){
    ...
    public function joint($data){
        ...
        if(!empty($data['goods_sn'])){
            if(!Goods::checkGoodsSn('ZH' . $data['goods_sn'])){
                $this->error = '商品编号已存在';
                return false;
            }
            $data['goods_sn'] = 'ZH' . $data['goods_sn'];
        }
...
//        dump($data);die;
        ...
        try {
            ...
            if(empty($data['goods_sn'])){
                $this->save(['goods_sn'=>Goods::makeGoodsSn($this->id,'ZH')]);
            }
            $this->addGoodsClass($data);
            // 商品规格
     ...
     * Time: 17:53
     */
    private function addGoodsClass($data,$isUpdate = false){
        $model = new GoodsAuxiliaryClass();
        $isUpdate && $model->remove($this['goods_sn']);
        $model->add($data,$this['goods_sn']);
    }
    ...
    public static function getGoodsByIds($data){
        ...
        original_img,is_free_shipping,is_recommend,is_new,is_hot,store_goods_type,suppliers_id,
