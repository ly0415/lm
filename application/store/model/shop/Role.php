...
use app\store\model\store\RoleAccess    as RoleAccessModel;
...
class Role extends RoleModel
{
    ...
    public function add($data)
    {
        ...
        try {
            ...
            // 新增角色权限关系记录
            (new RoleAccessModel)->add($this['role_id'], $data['access']);
            $this->commit();
            return true;
        } catch (\Exception $e) {
    ...
    public function edit($data)
    {
        ...
        try {
            ...
            $this->allowField(true)->save($data);
            // 更新角色权限关系记录
            (new RoleAccessModel)->edit($this['role_id'], $data['access']);
            $this->commit();
            return true;
        } catch (\Exception $e) {
            $this->error = $e->getMessage();
    ...
    public function remove()
    {
        ...
        // 删除对应的权限关系
        RoleAccessModel::deleteAll(['role_id' => $this['role_id']]);
        return $this->delete();
    }
     ...
     * @date    2019-09-02
     */
    public function getStoreRole($store_id){
        $StoreModel = new StoreModel;
        $store_info = $StoreModel::get($store_id);
        $business_id= $store_info['business_id'];
        if(!$business_id){
            return false;
        } else {
            $all = $this->getAll(['business_id'=>$business_id, 'type'=>2]);
            return $this->formatTreeData($all);
        }
    }
}