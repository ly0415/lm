...
class Order extends OrderModel{
    ...
    public function getList($search_store_id, $delivery_type, $start_time, $end_time, $phone, $order_sn, $order_state, $tips){
        ...
        //总站订单查询
        if($store_id == StoreModel::getAdminStoreId()){
            return $this->getStoreAdimList($store_id, $delivery_type, $start_time, $end_time, $phone, $order_sn, $order_state, $tips);
        }
     ...
     * @date     2019-12-05
     */
    public function getStoreAdimList($store_id, $delivery_type, $start_time, $end_time, $phone, $order_sn, $order_state, $tips)
    {

        $where['ore.mark'] = 1;
        $whereOr='';
        if(empty($delivery_type) && empty($start_time) && empty($end_time) && empty($phone) && empty($order_sn) && ($order_state == -1) && ($tips == 0) ){
            $whereOr['ore.sendout_sign']=1;
        }
        if($tips == 1){
            $where['ore.order_state'] = 20;
        }elseif($tips == 2){
            $where['ore.order_state'] = 60;
        }
        if(isset($order_state) && $order_state != '' && $order_state != '-1'){         //订单状态
//            $this->where('ore.order_state',$order_state);
            $where['ore.order_state'] = $order_state;
        }
        if(!empty($delivery_type)){         //配送属性
            $where['ore.sendout']     = $delivery_type;
        }
        if(!empty($start_time) || !empty($end_time)){            //下单时间
            $where = timeCond($where,'ore.add_time',$start_time, $end_time);
        }
        if(!empty($phone)){               //手机号
            $where['ore.phone']       = [ 'like', "%$phone%"];
        }
        if(!empty($order_sn)){            //订单号
            $where['ore.order_sn']    = [ 'like', "%$order_sn%"];
        }
        $result = $this
                ->alias('ore')
                ->field('ore.store_id ore_store_id,ore.order_sn ore_order_sn')
                ->where('ore.store_id',$store_id)
                ->where($where)
                ->whereOr($whereOr)
                ->order(['ore.order_id' => 'desc'])
                ->paginate(15, false, ['query' => \request()->request()])
                ->each(function ($value){
                        $prefix = Config::get('database.prefix');
                        Db::table( $prefix.'order_'.$value['ore_store_id'])
                        ->alias('a')
                        ->field('a.order_sn,a.store_id,a.add_time,a.sendout,a.buyer_id,a.order_state,a.order_amount,b.payment_type,c.pay_sn,c.sendout_time,c.number_order,d.username,d.phone,e.img source_img')
                        ->join('order_relation_'.$value['ore_store_id'].' b', 'b.order_sn = a.order_sn','LEFT')
                        ->join('order_details_'.$value['ore_store_id'].' c', 'c.order_sn = a.order_sn','LEFT')
                        ->join('user d', 'd.id = a.buyer_id','LEFT')
                        ->join('store_source e', 'e.id = c.store_source_id','LEFT')
                        ->where('a.order_sn',$value['ore_order_sn'])
                        ->select()
                        ->each(function ($item)use($value){
                            $it = $this->toSwitch($item, true);
                            $value['order_sn']                  =   !empty($it['order_sn'])?$it['order_sn']:'';
                            $value['store_id']                  =   !empty($it['store_id'])?$it['store_id']:'';
                            $value['add_time']                  =   !empty($it['add_time'])?$it['add_time']:'';
                            $value['sendout']                   =   !empty($it['sendout'])?$it['sendout']:'';
                            $value['buyer_id']                  =   !empty($it['buyer_id'])?$it['buyer_id']:'';
                            $value['order_state']               =   !empty($it['order_state'])?$it['order_state']:'';
                            $value['order_amount']              =   !empty($it['order_amount'])?$it['order_amount']:'';
                            $value['payment_type']              =   !empty($it['payment_type'])?$it['payment_type']:'';
                            $value['pay_sn']                    =   !empty($it['pay_sn'])?$it['pay_sn']:'';
                            $value['sendout_time']              =   !empty($it['sendout_time'])?$it['sendout_time']:'';
                            $value['number_order']              =   !empty($it['number_order'])?$it['number_order']:'';
                            $value['username']                  =   !empty($it['username'])?$it['username']:'';
                            $value['phone']                     =   !empty($it['phone'])?$it['phone']:'';
                            $value['source_img']                =   !empty($it['source_img'])?$it['source_img']:'';
                            $value['goods']                     =   !empty($it['goods'])?$it['goods']:'';
                            $value['format_delivery_type']      =   isset($it['format_delivery_type'])?$it['format_delivery_type']:'';
                            $value['format_store_name']         =   isset($it['format_store_name'])?$it['format_store_name']:'';
                            $value['format_order_state']        =   isset($it['format_order_state'])?$it['format_order_state']:'';
                            $value['format_payment_type']       =   isset($it['format_payment_type'])?$it['format_payment_type']:'';
                            $value['format_add_time']           =   isset($it['format_add_time'])?$it['format_add_time']:'---';
                            $value['format_sendout_time']       =   isset($it['format_sendout_time'])?$it['format_sendout_time']:'';
                            $value['format_phone']              =   isset($it['format_phone'])?$it['format_phone']:'';
                            $value['format_fx_user']            =   isset($it['format_fx_user'])?$it['format_fx_user']:'';
                        });
                });
        return $result;
    }
    ...
    public function getOrderDetail($order_sn){
        ...
        $result = Db::table( $prefix.'order_'.$orderInfo->store_id)
            ->alias('a')
            ->field('a.order_sn,a.store_id,a.add_time,a.sendout,a.buyer_id,a.order_state,a.goods_amount,a.order_amount,a.evaluation_state,a.source,b.pay_sn,b.sendout_time,b.number_order,b.seller_msg,b.delivery,b.shipping_fee,b.discount_num,b.discount,b.fx_money,c.payment_type,c.payment_time,c.receive_time,c.comment_time,d.username,d.phone,d.xcx_openid')
    ...
    public function toSwitch($value, $is_order_list = false){
        ...
        $value['format_store_name']  = isset($stores[$value['store_id']]) ? $stores[$value['store_id']]['store_name'] : '';
    ...
    public function getPrintData($order_sn){
        ...
        foreach ($print_data as $key => $val) {
            ...
            //获取桌号信息
            if( $val['sendout'] == 1 ){
                $print_data[$key]['format_table_type']  = $val['table_type'] ? $TableNumberModel->table_type[$val['table_type']] : '';
                $print_data[$key]['format_table_desc']  = $val['table_desc'] ? unserialize($val['table_desc']) : '';
            }
            //获取分销人员
            if( $val['fx_user_id'] ){
...
//        dump($storeUserName);die;
     ...
     * Time: 20:15
     */
    public function acceptOrder1($order,$store_user_id = 0){
        // 更新订单接单状态
        if ($status = $this->updateToTake($order,$store_user_id)) {
            // 发送消息通知
            $this->sendTakeMessage($order);
        }
        return $status;
     ...
     * Time: 20:15
     */
    public function acceptOrder($order,$store_user_id = 0){
        if (
            $order['order_state'] != 20
        ) {
            $this->error = '该订单不满足接单条件';
            return false;
        }
        Db::startTrans();
        try{
     ...
     * Time: 19:03
     */
    public function extractOrder($order,$source){
        // 更新订单接单状态
        if ($status = $this->verificationOrderSelf($order,$source)) {
            // 发送消息通知
            $this->sendTakeMessage($order);
        }
        return $status;
     ...
     * Time: 16:58
     */
    public function updateToTake($order,$store_user_id){
        if (
            $order['order_state'] != 20
        ) {
            $this->error = '该订单不满足接单条件';
     ...
     * Time: 16:59
     */
    private function sendTakeMessage($order)
    {
        // 实例化消息通知服务类
        $Service = new Message();
        // 发送消息通知
        $Service->take($order, OrderTypeEnum::MASTER);
        return true;
    }
