...
//        $user_id = 31643;
        $list =$this
            ->alias('g')
            ->field('re.c_money as re_c_money,re.s_money as re_s_money,re.integral as re_integral,re.percent as re_percent,
            u.username,u.phone,g.id,g.order_sn,g.type,g.status,g.c_money,g.old_money,
            g.new_money,g.point_rule_id,g.source,g.add_user,g.add_time,g.check_user,g.pay_time,g.transaction_id,g.mark')
            ->join('user u','u.id=g.add_user','LEFT')
            ->join('recharge_point re','re.id=g.point_rule_id','LEFT')
            ->where('g.add_user',$user_id)
            ->where('g.mark',1)
            ->order('g.add_time','desc')
            ->paginate(15, false, ['query' => \request()->request()])
            ->each(function($item){
                $this->getFormType($item);
            });
        return $list;
     ...
     * @date 2019-12-10
     */
    public function getbalanceHistoryListAll($user_id=''){
//        $user_id = 31643;
        $list =$this
            ->alias('g')
            ->field('re.c_money as re_c_money,re.s_money as re_s_money,re.integral as re_integral,re.percent as re_percent,
        ...
        if(!empty($item['type'])){
            switch($item['type']){
                case 1:
                    ...
                    break;
                case 2:
                    $item['description']  = '订单编号:'.$item['order_sn'];
                    $item['type_name']    = '消费扣除';
                    if(!empty($item['status'])){
                        switch($item['status']){
                            case 1;
                ...
                case 4:
                    ...
                    break;
                case 5:
                    $balancelist = Db::name('balance_recharge_coupon')->where(['use_user'=>$item['add_user'],'use_time'=>$item['add_time'],'mark'=>1])->find();
                    $code = isset($balancelist['sn'])?$balancelist['sn']:'';
                    $item['description'] = '充值码'.$code.'充值'.$item['c_money'];
                    $item['type_name']   = '充值码';
                    $item['c_money'] = '+'.$item['c_money'];
                    $item['type_status'] = '充值成功';
                    break;
                case 6:
                    $item['description']  = '退款订单号:'.$item['order_sn'];
                    $item['type_name']   = '订单退款';
                    $item['c_money'] = '+'.$item['c_money'];
                    $item['type_status'] = '已退款';
                    break;
                case 7:
                    ...
                    break;
                case 20:
                    $item['description'] = '余额冻结';
                    $item['type_name']   = '余额冻结';
                    $item['type_status'] = '已冻结';
                    break;
            }
        }
        return $item;
     ...
     * @date 2019-12-10
     */
    public function exportBalanceHistoryList($user_id=''){
        $list = $this->getbalanceHistoryListAll($user_id);
        $tileArray = ['标识', '变更金额', '变更前余额', '变更后余额', '类型', '状态', '操作时间'];
        $dataArray = [];
        foreach ($list as $coupon) {
            $dataArray[] = [
                '标识' => $this->filterValue($coupon['description']),
                '变更金额' => $this->filterValue($coupon['c_money']),
                '变更前余额' => $this->filterValue($coupon['old_money']),
                '变更后余额' => $this->filterValue($coupon['new_money']),
                '类型' => $this->filterValue($coupon['type_name']),
                '状态' => $this->filterValue($coupon['type_status']),
                '操作时间' =>date('Y-m-d', $coupon['add_time']),
            ];
        }
        // 导出csv文件
