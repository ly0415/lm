        ...
        if($storeGoods && $storeGoods['deduction'] == 1){
            ...
            if(!empty($spec)){
                ...
                $data['price'] = self::where('key','IN',$spec)
                    ...
                    ->value('price');
                $data['key_name'] = self::where('key','IN',$spec)
                    ->where('store_goods_id','=',$store_goods_id)
                    ->value('key_name');
                $data['key'] = self::where('key','IN',$spec)
                    ->where('store_goods_id','=',$store_goods_id)
                    ->value('key');
            }else{
                //无规格情况
            ...
            //有规格情况
            if(!empty($spec)){
                $data = self::field('goods_storage as stock,key,price,key_name')
                    ->where('key','IN',$spec)
                    ->where('store_goods_id','=',$store_goods_id)
                    ->find();
            }
            //无规格情况
            if(empty($spec)){
                $data = StoreGoods::getSpecPriceStock($store_goods_id);
                $data['key_name'] = '';
                $data['key'] = '';
            }
