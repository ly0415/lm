...
class GoodsCategory extends BaseModel
{
    ...
    public function roomCategory(){
        return $this->hasMany('RoomCategory','category_id','id')->order('sort ASC');
    }

    //商品辅助分类表
    public function goodsAuxiliaryClass(){
        return $this->hasMany('GoodsAuxiliaryClass','cate_id','id');
    }
    ...
    public function getImageAttr($val){
        if(!empty($val)){
            return ['big_file_path'=>'uploads/big/' . $val,'small_file_path'=>'uploads/small/' . $val,'value'=>$val];
        }
        return $val;
    }
